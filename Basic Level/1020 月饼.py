...
if __name__ == '__main__':
    ...
    inventory = list(map(float, input().split()))  # 正数
    total_prices = list(map(float, input().split()))  # 正数
    unit_prices = [p / i for p, i in zip(total_prices, inventory)]
    price = 0
    while d != 0:
        i = unit_prices.index(max(unit_prices))
        if d < inventory[i]:
            weight = d
            price += unit_prices[i] * weight
        else:
            weight = inventory[i]
            price += total_prices[i]
        d -= weight
        unit_prices[i] = 0
        total_prices[i] = 0
    print('%.2f' % price)
